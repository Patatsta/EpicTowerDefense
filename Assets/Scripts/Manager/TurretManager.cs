...
public class TurretManager : MonoBehaviour
{
    ...
    public GameObject upgradeButton, dismantelButton, hoverGate;
    ...
    private void Start()
    {
        hoverGate.SetActive(false);
        upgradeButton.SetActive(false);
        dismantelButton.SetActive(false);
    ...
    {
        if (IsEnabled) return;
        if (PlacementManager.Instance.turretStats[index].cost <= GameManager.Instance.warfunds)
        {
            GameManager.Instance.PlaceTurret(PlacementManager.Instance.turretStats[index].cost);
    ...
    {
        int refund = isUpgrade ? PlacementManager.Instance.turretStats[index].doubleRefund : PlacementManager.Instance.turretStats[index].singleRefund;
        print(refund);
    ...
    public void TogglePlaceHolder(bool state, int index)
    {
        if (!IsEnabled)
        {
            ...
            {
                case 0:
                    if (GameManager.Instance.warfunds >= PlacementManager.Instance.turretStats[index].cost)
